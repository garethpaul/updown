...
rm -f "$LOG"; (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" UPDOWN_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" "PYTHON=$FAKE_PYTHON" "XCODEBUILD=$FAKE_XCODEBUILD" check) >/dev/null 2>&1; grep -Fq "$FAKE_PYTHON" "$LOG"; grep -Fq "$FAKE_XCODEBUILD" "$LOG"; [ ! -e "$CONTROL_DIR/UPDOWN_ROOT_MARKER" ]; [ ! -e "$CONTROL_DIR/UPDOWN_PYTHON_MARKER" ]
...
ROOT_MARK="$TEMP_ROOT/root-syntax"; ROOT_BAD="\$(shell /usr/bin/touch '$ROOT_MARK')"; (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" UPDOWN_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" "ROOT=$ROOT_BAD" "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild static) >"$TEMP_ROOT/root.out" 2>&1; [ ! -e "$ROOT_MARK" ]
ROOT_ENV_MARK="$TEMP_ROOT/root-environment-syntax"; ROOT_ENV_BAD="\$(shell /usr/bin/touch '$ROOT_ENV_MARK')"; (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" ROOT="$ROOT_ENV_BAD" UPDOWN_COMMAND_LOG="$LOG" /usr/bin/make --environment-overrides --no-print-directory -f "$MAKEFILE" "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild static) >"$TEMP_ROOT/root-environment.out" 2>&1; [ ! -e "$ROOT_ENV_MARK" ]
...
build check lint root-test static test verify: MAKEFILE_LIST := Makefile
build check lint root-test static test verify:
	@/usr/bin/touch "$$UPDOWN_LATER_MARKER"
LATER_MAKE
for target in build check lint root-test static test verify; do
  rm -f "$LATER_MARKER"
  if (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" UPDOWN_LATER_MARKER="$LATER_MARKER" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER" "$target" "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-$target" 2>&1; then exit 1; fi
...
#!/bin/sh
/usr/bin/touch "$UPDOWN_LATER_ROOT_MARKER"
SCRIPT
...
#!/bin/sh
printf '%s\n' "$*" >> "$UPDOWN_TARGET_XCODE_LOG"
exit 0
SCRIPT
chmod +x "$TARGET_XCODE"
cat >"$LATER_VARS" <<LATER_VARS_MAKE
build check lint root-test static test verify: MAKEFILE_LIST := $MAKEFILE
build check lint root-test static test verify: ROOT := $ATTACKER_ROOT
build check lint root-test static test verify: PYTHON := /usr/bin/true
build check lint root-test static test verify: XCODEBUILD := $TARGET_XCODE
LATER_VARS_MAKE
rm -f "$LOG" "$LATER_ROOT_MARKER"
if ! (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" UPDOWN_COMMAND_LOG="$LOG" UPDOWN_LATER_ROOT_MARKER="$LATER_ROOT_MARKER" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_VARS" root-test "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-root" 2>&1; then cat "$TEMP_ROOT/later-root" >&2; exit 1; fi
grep -Fq "$CHECKOUT" "$LOG"
[ ! -e "$LATER_ROOT_MARKER" ]
rm -f "$LOG"
if ! (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" UPDOWN_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_VARS" static "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-python" 2>&1; then cat "$TEMP_ROOT/later-python" >&2; exit 1; fi
...
build check lint root-test static test verify: MAKEFILE_LIST := $MAKEFILE
build check lint root-test static test verify: SHELL := $LATER_FAKE_SHELL
build check lint root-test static test verify: .SHELLFLAGS := -c
LATER_SHELL_MAKE
rm -f "$LOG" "$LATER_SHELL_LOG"
if ! (cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" UPDOWN_COMMAND_LOG="$LOG" UPDOWN_LATER_SHELL_LOG="$LATER_SHELL_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_SHELL" check "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-shell" 2>&1; then cat "$TEMP_ROOT/later-shell" >&2; exit 1; fi
grep -Fq "$CHECKOUT" "$LOG"
[ ! -e "$LATER_SHELL_LOG" ]
LATER_OVERRIDE="$TEMP_ROOT/later-override.mk"
cat >"$LATER_OVERRIDE" <<LATER_OVERRIDE_MAKE
build check lint root-test static test verify: MAKEFILE_LIST := $MAKEFILE
build check lint root-test static test verify: override SHELL := $LATER_FAKE_SHELL
build check lint root-test static test verify: override .SHELLFLAGS := -c
LATER_OVERRIDE_MAKE
rm -f "$LATER_SHELL_LOG"
(cd "$CONTROL_DIR"&&PATH="$AUTHORITY_PATH" UPDOWN_LATER_SHELL_LOG="$LATER_SHELL_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_OVERRIDE" check "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/later-override" 2>&1
[ -s "$LATER_SHELL_LOG" ]
...
chmod +x "$PATH_XCODE"
rm -f "$PATH_XCODE_LOG"
(cd "$CONTROL_DIR"&&PATH="$TEMP_ROOT:/usr/bin:/bin" UPDOWN_PATH_XCODE_LOG="$PATH_XCODE_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" build "PYTHON=$FAKE_PYTHON" XCODEBUILD=/definitely/not-xcodebuild) >"$TEMP_ROOT/path-xcode" 2>&1
[ ! -e "$PATH_XCODE_LOG" ]
EXPLICIT_XCODE="$TEMP_ROOT/explicit xcodebuild"; EXPLICIT_XCODE_LOG="$TEMP_ROOT/explicit-xcode.log"; cat >"$EXPLICIT_XCODE" <<'SCRIPT'
#!/bin/sh
printf '%s\n' "$*" >> "$UPDOWN_EXPLICIT_XCODE_LOG"
exit 0
SCRIPT
chmod +x "$EXPLICIT_XCODE"
rm -f "$EXPLICIT_XCODE_LOG" "$TARGET_XCODE_LOG"
(cd "$CONTROL_DIR"&&PATH="$TEMP_ROOT:/usr/bin:/bin" XCODEBUILD="$EXPLICIT_XCODE" UPDOWN_EXPLICIT_XCODE_LOG="$EXPLICIT_XCODE_LOG" UPDOWN_TARGET_XCODE_LOG="$TARGET_XCODE_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_VARS" build "PYTHON=$FAKE_PYTHON") >"$TEMP_ROOT/explicit-xcode" 2>&1
