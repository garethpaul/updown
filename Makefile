...
override REPOSITORY_XCODEBUILD_IS_VALID := $(shell candidate='$(call REPOSITORY_PARSE_SHELL_LITERAL,$(XCODEBUILD))'; /bin/expr "$$candidate" : '^/' >/dev/null && /usr/bin/printf '%s' yes)
ifneq ($(REPOSITORY_XCODEBUILD_IS_VALID),yes)
$(error XCODEBUILD must be an absolute path)
endif
...
build check lint root-test static test verify:: $$(if $$(filter-out $$(REPOSITORY_MAKEFILE_LIST),$$(value MAKEFILE_LIST)),$$(error repository Makefile must be loaded alone))
build check lint root-test static test verify:: $$(if $$(filter-out $$(value MAKEFILE_LIST),$$(REPOSITORY_MAKEFILE_LIST)),$$(error repository Makefile must be loaded alone))
